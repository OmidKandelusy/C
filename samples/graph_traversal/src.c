...
void add_an_edge(void){
    ...
    printf("insert the starting node of the edge (the node's index starting from 0) \n\r");
    ...
    printf("insert the ending node of the edge (the node's index starting from 0) \n\r");
...
void traversal_bfs(void){
    ...
    while (dequeue_index < enqueue_index){
        ...
        for (int i=0; i<GRAPH_MAX_SIZE; i++){
            ...
            if (graph[current][i] == 1 && if_visited(visited, enqueue_index, i) == 0){
...
int search_unvisited_neighbor(uint8_t node, uint8_t *visited, uint8_t visited_len){
    for(int i=0; i<GRAPH_MAX_SIZE; i++){
