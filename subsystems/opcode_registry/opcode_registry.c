...
int register_opcode(char *opcode, opcode_callback_t opc_cb){
    ...
    int counter = 0;
    int erased_index = -1;
    while(current_slot.flag == OPC_SLOT_TAKEN || current_slot.flag == OPC_SLOT_ERASED){
        ...
        // check for duplicate:
        if (strcmp(current_slot.name, opcode)==0) break;
    ...
    if (erased_index != -1) index = erased_index;
...
int remove_opcode(char *opcode){
    ...
    if (opcode_register[index].flag != OPC_SLOT_TAKEN) return -5;
